...
function getPromise(number) {
  return new Promise((res, rej) => {
    ...
    rej(new Error('the number is negative'));
  });
}
...
function getFirstResolvedPromiseResult(promises) {
  return Promise.any(promises).catch(() =>
    Promise.reject(new Error('all promises in the array are rejected'))
  );
}
...
function getAllOrNothing(promises) {
  return Promise.all(promises).catch((reason) => Promise.reject(reason));
}
...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((res) =>
    res.map(({ status, value }) => (status === 'rejected' ? null : value))
  );
}
